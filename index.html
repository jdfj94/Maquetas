<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <!------- CSS -------->
    <link rel="stylesheet" href="Css/Maquetas.css">

    <!------- BOX ICONS -------->
    <link href='https://unpkg.com/boxicons@2.0.7/css/boxicons.min.css' rel='stylesheet'>

    <title>Maquetaciones de mis proyectos</title>
</head>

<body>


    <!------- HOME -------->

    <section class="home" id="principal">
        <h1 class="home__title">José David Frías Jiménez</h1>
        <div class="home__img">
            <img src="Imagenes/foto-perfil.jpg" alt="Foto de perfil José David">
        </div>
    </section>


    <main>

        <!------- PROYECTOS -------->

        <section class="section proyectos" id="proyectos">
            <h2 class="section-title">Maquetaciones en HTML y CSS</h2>
            <p class="texto__proyectos">A continuación puedes ver algunos ejemplos de maquetaciones realizadas con HTML
                y CSS. El objetivo es exclusivamente practicar y mejorar mis conocimientos en ambos lenguajes, no son proyectos
                entregados al cliente. Los originales los puedes ver en mi <a href="">CV</a>.</p>

            <div class="trabajos__container">

                <div class="trabajos__img">
                    <img src="Imagenes/proyecto-alex.jpg" alt="Proyecto alex photographer">

                    <a href="https://www.alxphotographer.com/" target="blank">
                        <div class="trabajos__data">
                            <span class="trabajos__titulo">Web de fotografía</span>
                            <i class='bx bx-link-alt trabajos__link'></i>
                        </div>
                    </a>
                </div>

                <div class="trabajos__img">
                    <img src="Imagenes/proyecto-legado.jpg" alt="Proyecto Legado Nazarí">

                    <a href="https://legadonazari.com/" target="blank">
                        <div class="trabajos__data">
                            <span class="trabajos__titulo">Tienda de botellas de aceite</span>
                            <i class='bx bx-link-alt trabajos__link'></i>
                        </div>
                    </a>
                </div>

            </div>

        </section>


    </main>



    <footer class="footer" id="footer">
        <div class="footer__container bd-grid">
            <h1 class="footer__titulo">José David Frías Jiménez</h1>

            <div class="footer__social">
                <a href="https://www.linkedin.com/in/jose-david-frias-jimenez/" target="blank" class="footer__link"><i
                        class='bx bxl-linkedin-square'></i></a>
            </div>
        </div>
    </footer>


</body>

</html>